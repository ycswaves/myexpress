...
describe("Implement Prefix Path Trimming", function(){
  ...
  it("should trims request path prefix when calling embedded app", function(done){
    request(app).get("/foo/bar").expect("embedded app: /bar").end(done);
  });

  it("should restore trimmed request path to original when going to the next middleware ensures leading slash", function(done){
